...
check("getSetting($this->contextId, 'googleApiKey')" not in (ROOT / 'classes/Jobs/SubmissionDiscoveryJob.php').read_text(encoding='utf-8'), 'submission discovery job bypasses encrypted API-key accessors')
check("getSetting($this->contextId, 'googleApiKey')" not in (ROOT / 'classes/Jobs/CatalogDiscoveryJob.php').read_text(encoding='utf-8'), 'catalog discovery job bypasses encrypted API-key accessors')
...
check('hasGoogleApiKey($this->contextId)' in submission_job, 'per-book post-crawl verification is scheduled without a Google API key')
