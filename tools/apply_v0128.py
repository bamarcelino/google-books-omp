...
p = root / 'classes/Model/BookMetadata.php'
...
replacement = "        $xml .= \"      </Language>\\n\";\n\n" \
    ...
    "        $xml .= \"    </DescriptiveDetail>\\n\";"
if marker not in s:
    raise SystemExit('Builder DescriptiveDetail marker not found')
s = s.replace(marker, replacement, 1)

marker = "        $xml .= \"    </PublishingDetail>\\n\";\n\n        if ($includeSupplyDetail) {"
replacement = "        $xml .= \"    </PublishingDetail>\\n\";\n\n" \
    "        $relatedProducts = [];\n" \
    ...
    "        if ($includeSupplyDetail) {"
if marker not in s:
    raise SystemExit('Builder PublishingDetail marker not found')
s = s.replace(marker, replacement, 1)
p.write_text(s, encoding='utf-8')

p = root / 'classes/Onix/GoogleOnixValidator.php'
s = p.read_text(encoding='utf-8')
marker = "        if ($book->seriesIdentifier !== null && trim((string) $book->seriesTitle) === '') {\n            $errors[] = 'Series title is required when a proprietary series identifier is supplied.';\n        }\n\n        return array_values(array_unique($errors));"
replacement = "        if ($book->seriesIdentifier !== null && trim((string) $book->seriesTitle) === '') {\n            $errors[] = 'Series title is required when a proprietary series identifier is supplied.';\n        }\n\n" \
    "        foreach ($book->subjects as $subject) {\n" \
    "            $scheme = trim((string) ($subject['scheme'] ?? ''));\n" \
    "            $code = trim((string) ($subject['code'] ?? ''));\n" \
    "            $heading = trim((string) ($subject['heading'] ?? ''));\n" \
...
s = p.read_text(encoding='utf-8')
marker = "check(str_contains($xml, '<SentDateTime>20260813T163045Z</SentDateTime>'), 'ONIX SentDateTime does not include precise UTC time');"
addition = marker + "\n" + r'''$enriched = clone $book;
$enriched->subjects = [
...
if marker not in s:
    raise SystemExit('run.php enrichment insertion marker not found')
